...
Two checks:
  ...
  1. ``test_q112_polynomial_gradient`` : full chain rule against autograd
     for the q_112 polynomial in isolation.
  2. ``test_full_angular_weight``       : ``_angular_weight`` matches
     ``torch.autograd.grad`` for a random Fp / s combo with every flag
     combination on the GPUMD-core blocks (q_222, q_1111, q_112).

The first pins down the specific polynomial coefficients hand-derived when
wiring the q_112 descriptor. The second confirms the full multi-block
combination (3-body + q_222 + q_1111 + q_112) sums correctly.
"""
import sys
