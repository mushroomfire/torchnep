...
def test_all_figures(tmp_path):
    ...
    fig = p.errors(tmp_path, out=tmp_path / "errors.png")
    assert len(fig.axes) == 4            # E / F / V histograms + force error vs magnitude
    fig = p.errors(tmp_path, quantities=("E", "S"), force_magnitude=False,
                   shift_energy="element", xyz=tmp_path / "data.xyz",
                   out=tmp_path / "errors2.png")
    ...
    for name in ("dash", "dash_density", "parity_scatter", "parity_density", "pred_scatter",
                 "pred_density", "shift", "excl", "loss", "errors", "errors2", "ptable", "ptable2"):
