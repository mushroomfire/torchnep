...
Cutoff rules, checked on load: angular cutoff ≥ 3 Å and ≤ the radial cutoff for every species, radial cutoff ≤ 100 Å, ZBL outer cutoff between 1 and 4 Å (GPUMD's limits); in addition the ZBL outer cutoff (`zbl`, every `zbl.in` row) must not exceed the smallest angular cutoff — ZBL pairs come from the angular neighbor list.
