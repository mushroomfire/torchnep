...
5. **No lint warnings.** Run `ruff check .` locally before pushing; it must report no warnings. The rules are in `pyproject.toml` (`[tool.ruff.lint]`); fix the code rather than adding exceptions.
